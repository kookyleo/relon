...
- [x] **语义校验增强**：将更多运行时的 `TypeMismatch` 提升到分析阶段（`relon-analyzer`）。
- [x] **文档提取 (Doc Extraction)**：从 `@schema` 和字段注释中自动提取元数据用于 LSP 悬停。
...
- [x] **参数化 Schema (Parameterized Schemas)**：深化泛型 Schema 支持，如 `@schema Page<T>: { List<T> items: * }`。
- [x] **品牌注册表 (BrandRegistry)**：正式化名义类型的运行时注册与查找机制。
