...
- **schema 字段**：所有声明字段（含 optional `T?` 与默认值字段）都
  纳入推导。Relon v1 没有「私有字段」概念 —— `with { ... }` 内部的
  method 不引入字段；schema 体里的字段全部对外可见、全部参与推导。
  未来若引入私有字段，再单独定义其参与策略。
...
#schema Money {
    ...
    #derive Equatable
    eq(other: Self) -> Bool:
        self.cents == other.cents and self.currency == other.currency
}
```
