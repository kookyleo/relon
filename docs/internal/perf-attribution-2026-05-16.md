...
## P2 阶段已落地（2026-05-16 追加）

### P2-A — Value enum 宽度收窄（merge `ae4d0fc`）

实测：`std::mem::size_of::<Value>()` **152 B → 24 B**。改造范围：

- Box 化 `Value::Closure` / `Schema` / `EnumSchema`（预期内）
- **额外**：`Value::Type(TypeNode)` 也 Box 化 —— TypeNode 单独就 ~100 B（`TokenRange + Vec<TypeNode>`），不 box 则 enum 卡在 ~120 B 无法过 48 B ceiling
- 新增 `value_enum_is_compact` `#[test]` 守 ≤ 48 B 上限
- 外部 carve-out：`crates/relon/src/projector.rs` 5 行 pattern 通配语法同步（唯一允许的非 evaluator 触碰）

P2-A 独立测量（vs post-P1-C baseline 460.7 MB）：

| 指标 | post-P1-C | post-P2-A 独跑 | delta |
| --- | --- | --- | --- |
| total bytes | 460.7 MB | 103.7 MB | -357 MB |
| at-gmax | 67.5 MB | **7.7 MB** | **-60 MB peak** |
| total blocks | 752 K | ~unchanged | 0（确认是 bucket width 而非 block count） |

at-gmax 下降是新发现的内存上限收益（peak 内存压力 -88%）。

### P2-B — comprehension hot loop 复用 Scope frame（merge `6e92436`）

实测命中 P1-B 留下的 48 MB / 200 K-block 钩子，**远超 -30 MB / -100 K 目标**：

| 指标 | post-P1-C | post-P2-B 独跑 | delta |
| --- | --- | --- | --- |
| total bytes | 460.7 MB | 140.0 MB | -320 MB |
| total blocks | 752 K | 353 K | -399 K / -53.1% |

实现路径（推荐方案，非降级）：

- `ListContext` 扩展 `iter_binding: Mutex<Option<(Arc<str>, Value)>>` + `index: Mutex<usize>` 槽位
- 新增 `Scope::with_iter_loop(elements)` 一次建外层 frame
- 新增 `Scope::set_iter_binding(name, val, idx)` 每元素 O(1) 刷新，无 `Arc<Scope>` alloc
- `Scope::get_local` 加 iter_binding fast path
- **闭包语义安全**：`Expr::Closure` 构造时沿 parent 链调 `snapshot_iter_bindings()` 把所有可见 iter binding 物化进 `captured_env`，无 iter binding 时短路成 `Arc::clone`（零开销）；支持嵌套 comprehension 内层 shadowing

### P2 组合效果（实测）

两者在同一 main 上的 dhat 真实组合：

| 指标 | P0 baseline | post-P1-C | post-P2 (combined) | 端到端 delta |
| --- | --- | --- | --- | --- |
| total bytes | 540 MB | 460.7 MB | **19.8 MB** | **-96.3% / 27× 缩减** |
| total blocks | 1.55 M | 752 K | **353 K** | -77% / 4.4× 缩减 |
| at-gmax | 67.5 MB | 67.5 MB | **7.7 MB** | -88% peak |

组合 -441 MB 超过两者独立之和（-357 + -320 = -677 但起点是同一个 460.7 MB —— 真实组合 460.7 → 19.8 = -441 MB），**协同效应是乘性的**：

- P2-B 砍掉每元素 HashMap 整体 → 没有桶表分配了
- P2-A 让剩下每个 `Value` 体积 6.3× 缩小（24 / 152）
- 二者作用域正交：B 砍 block 计数，A 砍 byte/block

### P3 候选（未启动）

1. **CPU profile 补齐**——离线 host 调 `perf_event_paranoid ≤ 1` 跑 flamegraph，确认 `BTreeMap → IndexMap` / `path_cache_key` 的 CPU 占比
2. **wall-time bench 对照**——criterion 跑 baseline vs post-P2 端到端 wall time（dhat 仅测 alloc，没测实际延迟）
3. **at-end 残留 ~7.7 MB**——已是 Context / Evaluator / stdlib 注册表常驻，进一步压需要 Context 复用 API 设计
