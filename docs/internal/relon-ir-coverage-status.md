...
- ✅ **值→String 统一分发 Wave A(`ebe82085`)**:lowering 新增 `lower_value_to_string(ir_ty)` 单点分发器,f-string 插值与 `String + 非String` 拼接两个消费方统一接入——String 恒等、I64 走既有 `Op::IntToStr`(字节不变,GENERATOR_VERSION 未 bump)、**Bool 编译落地**(`Op::If` 选 "true"/"false" 常量串,无新 op),F64 与复合类型响亮 cap。配套 analyzer 修正:oracle 本就接受 `String + 任意`/`任意 + String` 为拼接(arithmetic.rs 双侧 arm),analyzer 此前把 `String + Bool` 静态拒掉属不自洽,现对齐。aot_wasm_parity 111(fstring_bool / string_plus_bool / bool_plus_string 四方)。**Wave B(进行中方向)**:`Op::FloatToStr` 经共享 Rust leaf helper(与 oracle 同一 `format!` 渲染),封掉 pricing `@currency` 拼 Float 价格的最后缺口。
...
| spread `...x` | `eval.rs` `Expr::Spread`(~681) | **已编:list 字面量源(R12)、dict schema 源(R12)、单运行期列表源 `[a?, ...xs:List<Int/Float>, b?]`(`e8787c29`,scratch materialize + MemoryCopy,无新 Op)** | 仍 cap:多运行期源 `[...xs,...ys]`(需循环累加,后置)、`List<String>`/`List<Schema>` 源、匿名 Dict 源(by-design)。注:`List<Float>` 运行期源 tw==cranelift+CLI 验证,wasm leg 同既存 ListFloat decode-臂缺口(非新 miscompile) |
...
| decorator `@foo(...)` | tree-walk(pricing 用 `@currency`) | **已编 scalar-Int(R11) + 纯-String 结果(`1d62910d`,closure 字段 ret_ty 从类型系统取=标注优先/保守 body 推断,替掉写死 I64)**;desugar `@deco(args) k: v ≡ deco(v, args)`(value-first),stacked 自底向上 | 仍 cap:builtin `@`-decorator、named arg、multi-segment、branded `-> Schema` 字段;**含 Float 操作数的 String 拼接**(如 pricing `@currency` 拼 Float 价格)——Int/Bool→String 已由 Wave A 统一分发器封掉,剩 Float 因编译版**无 Float→String 转换 op**,响亮 cap 非静默;Wave B 以 `Op::FloatToStr`+共享 Rust helper(同 oracle `format!`)补齐 |
