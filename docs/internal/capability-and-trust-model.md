...
- **§5 那张表现在描述的是「机制就位 **且** 入口已通」**(bytecode 全通;cranelift 见 §9.3)。

### 9.3 还没接上的尾巴(诚实记录)

- **cranelift 的 host-fn dispatch(grant 路径)未通**。producer 已发 `Op::CheckCap` + `Op::CallNative`,
  cranelift 的 consumer 也早已 codegen 这两个 op,所以 **deny 路径在 cranelift 上端到端可达**:
  `from_source_with_options` 编译后,未注册 cap slot 的 `CheckCap` 在运行期 trap `CapabilityDenied`
  (测试见 `tests/native_call_from_source.rs`)。但 **granted 的真实 dispatch 还差两块**:① cranelift 的
  vtable 是单个 `Vec<Option<HostFnPtr>>`,被 `CheckCap`(按 cap_bit 索引)与 `CallNative`(按 import_idx
  索引)**共用一个命名空间**——低位 import_idx 会与 cap_bit 0–5 撞槽,需把 import-slot 与 cap-bit 槽位
  分离;② `HostFnPtr = extern "C" fn(i64)->i64` 是裸函数指针,无法承载动态 `Arc<dyn RelonFunction>`,
  把 tree-walker 的 Arc 注册进 cranelift 需要一个 codegen 透传 slot/state 的通用 trampoline,或每-fn 的
  JIT trampoline。这是 **host-SDK ABI 里程碑**(与 bytecode 的 `host_fns` map 模型本质不同),独立排期。
- bytecode 的 `CallNative` dispatch 仍是 **phase-4a**:实参只走 Int lane(`Value::Int`),richer 实参类型
  待 buffer-protocol 信封(phase-4b)。String 返回值已能 lift 进 arena;String/List 实参未通。
