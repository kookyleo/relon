...
> 这三种 `CapabilityDenied` / `WasmCapabilityDenied` 历史上是两个不同的 error 形状 + 两处审计点;
> `capability.rs` 的 `CapabilityGate` 把策略统一成"宿主授了这个 bit 吗"一个决定,各后端只是**消费**它、
> 保留各自的 enforcement timing。
