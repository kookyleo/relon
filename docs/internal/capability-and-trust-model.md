...
- **`Op::CheckCap` / `Op::CallNative` 没有 源码→IR 的 producer**。三套 consumer 齐备且有测试
  (cranelift codegen、bytecode compile、trace-recorder),但 `relon-ir/lowering.rs` 至今不生成它们
  ——属 v5-γ(`Op::CallNative` / `Op::CallClosure` 一并落地)。**所以 cranelift / bytecode 的 cap 强制
  目前端到端不可达**,只能吃手搓 IR;§5 那张表描述的是机制就位、入口未通。这也是为什么 §9 的
  cranelift `--trust` 是 moot——不只是"CLI 没 registry",而是整条 native-call lowering 还没有 front-end。
  这是**功能未完成**(有路线图),不是可删的死代码,所以只记录、不动。
