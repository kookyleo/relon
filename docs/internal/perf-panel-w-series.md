...
- W12 fast 入口真回退：2.8915 → 7.7313 ns（2.67×），bisect culprit =
  `7a908521`；**已修复**（fix `1779a99d`，2026-06-10）：s90 复测 fast 入口
  3.38 ns，与同二进制 rust 路径 3.40 ns 恢复平价。机制 = `expect` panic 垫
  把 dispatch 推过 fat-LTO 内联阈值，见「W12 llvm_aot_fast 回退定位」修复段。
...
本行保留 2026-06-10 快照原数；回退已于同日修复（fix `1779a99d`，s90 复测
3.38 ns），见「W12 llvm_aot_fast 回退定位」修复段。
...
§ 2026-06-10 快照原数，回退已于同日修复（fix `1779a99d`，s90 复测 3.38 ns、
13.07 G instructions），见「W12 llvm_aot_fast 回退定位」修复段。
...
#### 修复（2026-06-10，fix `1779a99d`）

- **机制（对照实验证实，非猜测）**：`7a908521` 把
  `run_main_legacy_i64_fast` / `try_run_main_fast` 里 `fast_path_arity` 的
  `unwrap_or(0)` 换成带消息的 `expect(...)`，在函数体内嵌入 panic 垫
  （`expect_failed` 调用 + 消息与 `Location` 实参装配）。fat-LTO 下该函数
  inline cost 越过阈值，**调用方不再内联 dispatch**：每次调用付出外联
  call/ret + `Result<i64>` 信封经栈内存往返 + 哨兵 discriminant 比较 +
  无法外提的 Option/arity 检查。证据：
  - 仅把两处 `expect` 改回无 panic 路径（其余 zero diff），本地
    9.46 → 3.55 ns，单因素复原；
  - objdump（保留符号构建）：culprit 树 bench 主函数对
    `run_main_legacy_i64_fast` 有外联调用点（热循环内每迭代 1 次 call +
    `movabs` 哨兵比较）；无 panic 垫版本热循环被内联成 7 条指令
    （存参 → `call *%r15` → checksum 累加 → 循环计数）；
  - s90 `perf stat`（1e9 次调用）：每调用 45 → 13 instructions、
    16.1 → 7.1 cycles、12 → 4 branches。
- **修复方式**：`fast_path_arity` 物化为非 `Option` 的 `usize` 字段，与
  `fast_entry_ptr` 在唯一 resolution site 一次性成对赋值——
  「指针在场、arity 缺失」状态在类型上不可表示，无需运行期校验也无静默
  吞值；热路径零 `Option` unwrap、零 panic 垫。arity 不匹配仍在入口处
  响亮报错（`Err`，路径冷）；公开 getter `fast_path_arity()` 仍返回
  `Option<usize>`（由指针在场性导出），对外语义不变。
- **s90 复测**（taskset -c 2，1e9 iters × 3 runs 取中位，UTC 06-10）：
  - culprit 二进制（`8c00b59a` 树）：7.72 ns —— 与 panel 快照 7.73 ns 吻合；
  - fix 二进制：**3.38 ns**，与同二进制 rust 路径 3.40 ns **恢复平价**
    （5/30 的 2.89 ns 与本次 3.38 ns 间的差异属 bench binary 快照间布局
    差异，按诚实口径以同二进制 fast-vs-rust 平价为判据）。
- **布局敏感性记录**：本地（非 s90）fix 后 4.15 ns vs 实验性
  `unwrap_or(0)` 版 3.55 ns——objdump 比对两者热循环**指令逐条相同**，
  仅循环头对齐不同（mod64 = 48 vs 0，48 字节循环体跨缓存行），属 bench
  二进制布局蝴蝶，非引擎侧 per-call 工作；s90 上无此残差。
- 抽测无连带回退（本地，同协议 A/B）：W7 fast 104.32 → 104.27 µs、
  W17 fast 2754 → 2748 ns、W18 fast 921.3 → 920.9 µs。
