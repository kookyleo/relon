...
| 支柱 | 角色 | 由谁演化 | 类比 |
|---|---|---|---|
| **P1. interpreter + trace-JIT** | 通用运行时核心:**搞定所有语义**。解释器跑冷路径 / 片段求值 / 数据依赖分发 / 作 oracle;trace-JIT 投机编译热路径,守卫失败 deopt 回解释器 | tier ① + tier ③ 的 **trace-JIT 部分** | **LuaJIT**(解释器 + trace-JIT) |
...
### 关键澄清:cranelift 不删,而是**降格专用**
- cranelift **不再是通用 AOT 后端**(那个角色让位:静态→llvm,动态→wasm)。
- cranelift **保留为 P1 里 trace-JIT 的码生成器**(录制器 → cranelift 发射 trace fn → 安装)。trace-JIT 本就需要一个快速码生成器,cranelift 正合适(纯 Rust、零工具链、快编)。
- **bytecode(tier ②)删除**:它的「deopt 落点」角色由 P1 的解释器直接承担(现状 trace-JIT 本就 bottom-out 到 tree-walk,而非 bytecode)。

### 目标后端集
> **从 5 条 →「3 支柱 / 4 crate」**:
> - P1 = `relon-evaluator`(解释器)+ `relon-codegen-cranelift`(降格为 trace 发射器)+ `relon-trace-*`(录制/优化/发射)
...
| 场景 | 支柱 |
|---|---|
| 桌面 / 服务端,运行时甩字符串、要正确 + 自适应快 | **P1**(解释器冷启动 + trace-JIT 热路径) |
| 交互式工具(LSP 行内求值、REPL) | **P1** 的解释器(片段求值) |
| 数据依赖的闭包 / 多态派发要跑快 | **P1** 的 trace-JIT(投机 + deopt) |
...
5. **cranelift 降格**:当 llvm/wasm 接管通用 AOT 后,cranelift 收缩为 trace-JIT 专用发射器。
...
> **3 支柱 = LuaJIT 式自适应核心(P1)+ 静态原生 AOT(P2:llvm/relon-rs)+ 动态可移植 JIT(P3:wasm)。** 它们对应三种**部署形态与信任姿态**,而非三个重复的编译器。cranelift 不死,降为 P1 trace-JIT 的码生成器;bytecode 退役。剩下的不是「能不能」,是 §6 那几个工程/产品决断。
