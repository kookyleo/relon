...
- **状态**:方向已认可,实施待排期(2026-06-03);**已据代码核对修正**(见 §0)
...
| 支柱 | 角色 | 由谁演化 | 类比 |
|---|---|---|---|
| **P1. 自适应核心** | 通用运行时核心:**搞定所有语义、且今天即全覆盖**。核心两挡——**解释**(冷路径 / 片段求值 / 数据依赖分发 / 作 oracle)· **cranelift 直连 AOT**(非平凡 run_main:全覆盖,含闭包表/native 调用)。**外加可选第三挡 trace-JIT**(窄:仅无分支数值热循环;见 §0 C1) | tier ① + tier ③ **全部** | 核心 = 解释 + AOT;trace-JIT = 实验性投机层(非 LuaJIT 级,recorder 待补) |
...
> **3 支柱 = 自适应核心(P1:解释 + cranelift 直连 AOT;trace-JIT 为可选窄优化)+ 静态原生 AOT(P2:llvm/relon-rs + Phase C 能力门)+ 动态可移植 JIT(P3:wasm)。** 它们对应三种**部署形态与信任姿态**,而非三个重复的编译器。**性能主力是 AOT(直连/llvm/wasm),不是 trace-JIT**;**功能完备 + 全覆盖由 P1 今天就托底(无需生长)**;cranelift 不死,降为 P1 内部的「直连 AOT + trace 发射器」两职;bytecode 退役(前置 §0 C3)。剩下的不是「能不能」,是 §6.1 那几个工程/产品决断 + §0 那几个前置。
