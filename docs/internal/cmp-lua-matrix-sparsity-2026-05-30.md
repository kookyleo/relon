...
| W20_n_body | 212.040µs | 240.080ms | — | — | **53.877µs**⁸ | — | — ᶠˡ | — | 239.690ms | 25.156µs |
...
| **参与数 /28** | **28** | 17² | **12** | **9** | **8**⁸ | **4** | **5**⁷ | **10** | **28** | **8**⁹ |
...
诚实定位:W28 近平价(1.076×);W20 2.14× 源于每步不可变列表分配(镜像 tree-walker 语义,Rust 用可变
in-place 状态)—— 语义不对称非 codegen 缺陷;两者皆 bit-exact、皆胜 luajit。followup:W20 cranelift
(`If result_ty F64`)、mixed-Mod 提升、computed ListInt 字面量、n>>14k 的 per-iter scratch reset。
