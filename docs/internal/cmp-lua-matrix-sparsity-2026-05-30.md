...
`¹⁰` W20 cranelift n/a→332.10µs(#361)。followup(未做):n>>14k 的 per-iter scratch reset。
...
### completeness followups(DONE,无 panel 行变化)
两项「无现有 workload 需要」的完整性补全,并行 worktree lane + 对抗审查双 accept,均 oracle bit-exact:

- **computed List<Int> 字面量 AOT 物化** `feat(ir): materialize computed List<Int> literals through AOT`(`1ee2de3c`)——
  补上 #359 Part B 只做了 Float 的那半:computed Int 列表字面量 `[e0,e1,...]` 现经 `emit_list_int_literal_materialize`
  物化(AllocScratchDyn + i32 header + 逐元素 `StoreI64AtAbsolute`,布局与 ConstListInt/range-map/`lower_list_int_index`
  完全一致),**无新 Op、无后端改动**(StoreI64AtAbsolute/AllocScratchDyn 各后端早已实现)。oracle bit-exact vs
  tree-walker(含 n=0 手算 + 负值)。诚实边界:analyzer 把裸列表字面量推断为 Tuple,故直接 `xs[0]+xs[1]` 形态被
  analyzer 拒(超出本 lane 的 ir-lowering scope),测试改用语义等价的闭包参数 shape(work item 列出的有效覆盖),
  仍是真 oracle 对比、非 paper-over。

- **cranelift F64 Mod via fmod libcall** `feat(cranelift): lower F64 Mod via fmod libcall with zero-divisor trap`(`feb50d42`)——
  把 #362 里 cranelift 对 `Op::Mod(F64)` 的优雅拒绝升级为真支持:cranelift 无原生 frem,故声明外部 `fmod`
  (f64,f64)->f64(Linkage::Import)、每个 body import FuncRef、`ins().call` 调用;JIT 经 `JITBuilder::symbol("fmod",
  relon_fmod)` 解析(`relon_fmod` 即 Rust `a % b`=f64::rem=tree-walker 同款,故 bit-identity by construction)。
  零除 trap 与 `emit_div_f64` 字节一致(`FloatCC::Equal` vs 0.0,捕获 ±0.0、不捕获 NaN,trap 在 fmod 调用前)。
  oracle bit-exact(f64::to_bits)含负被除数/负除数/分数除数/|被除数|<除数 边界 + ±0.0 两引擎都 `DivisionByZero`。
  object/dlopen 路径也声明了 `fmod` import(correct-by-construction,无测试覆盖,已诚实标注)。

剩余 followup(未做):n>>14k 的 per-iter scratch reset(W20 式 list-reduce 的 O(n)→O(1) arena 占用,真实扩展性
但触及 delicate arena 模型、W20_N=1000 远未触界);wasm F64 Mod(本就 scope-cut 所有 f64,保持优雅拒绝)。
