...
- [ ] **stdlib type schema 载体（follow-up）**：让用户写 `s.upper()`
      ...
      method 声明。
- [ ] **constraint lowering 扩展**：Iterable（`for`/`map` 上的隐式
      调度）、Indexable（`a[i]`）、Callable（`f(...)`）、Number
      （算术运算）当前仅在 `constraints.rs` 留注释；需要分别在
      evaluator 的 list iteration / index expression / FnCall /
      arithmetic 子系统加 method dispatch 兜底。
