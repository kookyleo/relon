...
- [ ] **P0：Capability model hardening**：当前 `NativeFnGate` /
      `Capabilities` 只覆盖 `reads_fs`，但文档和真实嵌入场景已经在
      讨论 `network`、`reads_clock`、`reads_env`、`uses_rng`、
      `writes_fs` 等 ambient state。需要同步扩展 evaluator、analyzer
      mirror type、静态 `CapabilityRequired` 诊断、runtime gate 和
      文档示例。
- [ ] **P1：stdlib 纯度守门**：现有 stdlib intrinsic 实际为纯函数，
      但缺少机器守门。短期可加测试 / lint 禁止 `stdlib.rs` 直接使用
      `std::fs`、`std::env`、`std::net`、时钟、RNG、process 等 ambient
      API；未来 `std/time`、`std/env` 类能力必须走 gated host-facing
      module。
- [ ] **P1：注册 API 语义收口**：保留 `register_fn` 兼容性，但文档
      和 API 应更明确地区分 trusted pure fn 与 ambient / side-effect
      fn；后者默认走 `register_fn_with_caps`。
...
- [ ] **trait bound 系统**：给 stdlib `len<T>` 加 `T: Stringifiable`
      之类的约束。这是 v2.0 范畴；设计草案见
      [`type-constraints-spec.md`](./type-constraints-spec.md)。
...
- **当前测试总数**：761 项（`cargo test -- --list`，2026-05-10）
- **最近全量测试**：`cargo test` 全绿（2026-05-10）
