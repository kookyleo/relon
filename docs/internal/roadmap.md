...
- [x] **Phase C（部分）**：comparison operator 下沉到 schema-rooted
      witness — `==` / `!=` 走 `eq`，`<` / `>` 走 `lt`（`>` 等价
      `rhs.lt(lhs)`）。primitives 与无 brand 的值仍走原 built-in 路径。
- [x] **Phase D（API）**：`Context::register_method(schema, name, gate, func)`
      ...
      到 source-side body。
- [ ] **Phase D 收尾**：把现有 36 条 stdlib intrinsic 从 `register_fn`
      迁移到 `register_method` + `std_relon/<type>.relon` schema 声明
      载体；保留 `register_fn` / `register_pure_fn` 作为 free-fn 路径。
- [ ] **Phase C 收尾**：constraint 模型（Equatable / Comparable /
      JsonProjectable / Iterable / Indexable / Callable / Number）正
      式登记；auto-derive Equatable/JsonProjectable，opt-in
      Comparable；`#derive` 在 method 上做 witness 检查；`<=` `>=`
      合成。
- [ ] **多段 path 中段 schema 类型纯度**：目前 `pkg.X.field` 这种
      三段及以上的路径仍然是 alias-or-nothing；下一段（field 名）
      静态推断仍要落 Any 兜底。
- [ ] **多段 method dispatch**：`obj.field.method()` 仍只识别
      2-segment `head.method` 形式；3-segment 需要 path-tail 推断
      与 receiver 解耦。

## 测试基线

- **当前测试总数**：807 项（`cargo test`，2026-05-11，schema-rooted
  Phase A.1 / B / C / D API 落地后）
