...
- [ ] **多段 path 中段 schema 类型纯度**：`pkg.X.field` 这种三段及以
      上的路径中段静态推断仍落 Any 兜底（与 method dispatch 的 prefix
      推断不是同一条 path —— prefix 推断借助 schema_methods 表，中段
      字段访问仍走原 walk_path 的 PathTailOutcome）。
- [ ] **method generics 的运行时 unification**：`Indexable.index(key: K)
      ...
      pass 是自然落点，无须 IR 改动。详见 §C.13 末尾。
- [ ] **method generic 与 schema generic 同名 warning**：`#schema
      List<T> with { foo<T>(...) }` 当前不报错但 substitution 顺序
      可能产生混乱。后续可加 `MethodGenericShadowsSchemaGeneric` warning
      diagnostic。详见 §C.12 末尾。
- [ ] **Iter cursor leak**：stdlib-local cursor 表在进程内永不释放，
...
- **当前测试总数**：855 项（`cargo test`，2026-05-11，schema-rooted
  Phase A.1 / B / C / D 全部落地，所有 §J follow-up 关闭）
