...
| `relon-eval-api` | The seam between hosts and evaluation backends: value model, host configuration, sandbox surface, and the object-safe evaluation contract — the backend-agnostic `Evaluator` core (`eval_root` / `run_main`) every backend implements, plus the `TreeWalkEval` extension (`eval` / `force_thunk` / `invoke_closure`) only tree-walking implementations can provide. Shared by the tree-walk evaluator, both AOT backends, and the wasm host wrapper | `Value`, `Scope`, `Thunk`, `RuntimeError`, `Context`, `Capabilities`, `CapabilityGate`, `Evaluator`, `TreeWalkEval` |
