...
## `#import` 链上的严格性（按模块各管各的）

严格性是**按模块（文件级）**的：每个模块默认严格，只能用它**自己**的 `#relaxed` / `#unstrict` 退出。一个模块的指令只管它自己——入口的 `#relaxed` **不会**被印到它 `#import` 的库上。

- 默认（无指令）的库始终按严格分析，无论谁导入它——`#relaxed` 入口也不会把它放松。
- 库要想放松，必须在库文件**自己**头部写 `#relaxed`；这条放松只作用于该库本身。

那「整程序不漏 `Any`」的保证靠什么？靠 **边界拦截**，不是靠传染：一个严格模块不会静态接受 `#relaxed` 依赖产出的 `Any`——它会在**使用点**报错（被依赖的库本身既不会被重新检查，也不会被消费者放松）。

这与主流语言一致：TypeScript、Rust、Sorbet、C#、Dart、JS 的严格性都是按文件/按模块自选；Safe Haskell 则把严格性做成「向内的 import 约束」。消费者从不替依赖降级。
...
- workspace 里的共享库——库默认就严格（除非自己写 `#relaxed`），所以库里不能藏 silent fallback；这与谁导入它无关。
