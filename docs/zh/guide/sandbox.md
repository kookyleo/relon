...
`Context` 提供两个语义化构造器，差别完全在**各 capability 的默认值**：

**文件读**
- `trusted()`：全开（`FilesystemModuleResolver::trusted()`）。
- `sandboxed()`：默认拒绝（`FilesystemModuleResolver::default()`，无 root）。

**步数预算 / Value 大小预算**
- 两者**都默认无限**——`sandboxed()` 也不替你拍预算。理由：合理预算高度依赖业务场景，没有合适的「安全默认」，留给宿主显式决定。

**`register_fn` 注册的函数**
- 两者都全放——这类函数被视作「宿主已认证」。

**`register_fn_with_caps` 注册的「门控」函数**
- `trusted()`：全放（`allow_all_native_fn = true`）。
- `sandboxed()`：默认拒绝；要放行得把名字加进 `allow_native_fn`。

**典型场景**
- `trusted()`：自己人写的脚本、CLI、构建期。
- `sandboxed()`：用户脚本、SaaS 配置、多租户。
