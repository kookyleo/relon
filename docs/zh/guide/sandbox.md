...
`Capabilities` 就是这个授权通道。规则不是「不能存在 trusted API」，
而是**没有隐式 trust**：host fn gate、import resolver 和运行策略必须
显式写出需要哪些能力，宿主显式决定授予哪些；脚本不能给自己授权。任何全信任 host 路径（`--trust`、`*_trusted`、
`TrustLevel::Trusted`）都只用于 host-owned 脚本，并且必须在调用点
可见。

本页是操作指南。权威的安全边界声明在 [威胁模型](./threat-model.md)：
`Capabilities` 是权限词汇，`ResourceBudget` 是标准预算模型；不可信
部署的强隔离应由 Wasmtime、进程边界或宿主基础设施执行。
...
## 不可信脚本默认入口：`Context::sandboxed()`

不可信或用户编写的脚本应使用 `Context::sandboxed()`：默认零特权，
宿主必须显式授权才能放行。
...
// 授权先在一个 `Capabilities` 值上拼好，再在构造期一次性安装。
// `Context` 不暴露运行中改能力的入口——构造完交给求值器后，授权
// 就是不可变的策略。
let mut caps = Capabilities::default();
let budget = ResourceBudget::untrusted();

// 1. 文件读 bit（如果脚本需要 #import 别的 .relon）
caps.reads_fs = true;

// 2. 安装 evaluator 侧预算。这里把新的 ResourceBudget 模型
// 桥接到求值器今天仍读取的兼容字段上。
budget.apply_to_capabilities(&mut caps);

// 3. 给确实需要的 bit 授权（按需开放，例如读时钟）
caps.reads_clock = true;

let mut ctx = Context::sandboxed().with_capabilities(caps);

// 文件读还需要一个带 root 的 resolver（bit 是策略，resolver 是机制）
...
`--max-source-bytes` 会先用文件 metadata 检查源码大小，再读入和解析，因此
能在 parser 递归参与前拒绝病态输入。CLI 的 evaluator 侧预算在
`--backend auto` 下会强制走 tree-walk；如果显式写了
`--backend cranelift-aot` 则直接拒绝，避免限制被静默忽略。
`--max-output-bytes` 在 CLI JSON 输出边界执行，不依赖 evaluator 后端。

VM 部署不要把预算塞进 Relon 源文件，而是生成宿主 runtime 策略：

```bash
relon host-policy --target wasmtime --profile untrusted
```

边界模型见 [威胁模型](./threat-model.md)；runtime 侧接线见
[Wasmtime 宿主策略](./wasmtime-host-policy)。

### 「我就是想全开」——`Capabilities::all_granted()`

宿主自己写的脚本（CLI、构建期、host-owned 配置文件）当然有权访问
所有能力。规范要求**这个授权必须显式可见、可审计**。完全写开的形
态是：
...
这两行表示「FS 全开 + 六个能力 bit 全开（任何 gate 都过）+ 无步数
预算」。facade 也提供带 trust 命名的 host-owned 捷径；只有在全开姿
态是有意选择、且 code review 一眼可见时才使用。
...
`max_steps` 和 `max_value_elements` 仍留在 `Capabilities` 上，是当前
求值器读取的兼容载体。新的宿主代码优先使用 `ResourceBudget`，再用
`ResourceBudget::apply_to_capabilities` 桥接：
...
- **`Capabilities`**（宿主授予）：在 `Capabilities` 值上置
  `network = true` 再经 `with_capabilities` 安装，表示「这个
  context 允许做网络」；
...
fn run_user_rule(
    ...
    // 求值预算（文件读保持关闭：`Context::sandboxed()` 的默认
    // resolver 链就是「只认 std/ 虚拟模块、真实路径一律拒绝」，
    // 不开 reads_fs、不挂 with_root_dir resolver 即可）
    let mut caps = Capabilities::default();
    ResourceBudget {
...
- ❌ **CPU 挂钟时间限制**：Relon 没有内建 wall-clock budget。如果你
  ...
  `tokio::time::timeout` / 单独线程 + 超时通道实现。Wasmtime 场景用
  engine/store 限制和宿主 epoch 递增，见
  [Wasmtime 宿主策略](./wasmtime-host-policy)。
