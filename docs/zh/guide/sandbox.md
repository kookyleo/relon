...
| --- | --- | --- |
| 文件读 | 全开（`FilesystemModuleResolver::trusted()`） | 默认拒绝（`FilesystemModuleResolver::new()`，无 root） |
| 步数预算 | 无限 | 无限（你需要显式设） |
...
### `fs_read_allowlist: Vec<PathBuf>`

文件读白名单的**记录性字段**——给宿主自己（或诊断/审计）看的。**实际的拒绝行为**由 `FilesystemModuleResolver::with_root_dir(...)` 在 resolver 层强制——见下一节。一句话：要拒文件读，靠 resolver 层；这个字段是元数据。
...
Default-constructed (`FilesystemModuleResolver::new()`) 的 resolver **任何**路径都拒——这是 `Context::sandboxed()` 给你的默认。
