...
每个 `.relon` 文件求值后产出一个 JSON 值——Object、Array、String、Number、Bool 或 Null。**根可以是任意表达式**：dict / list / tuple literal、原子字面量、二元 / 三元 / pipe 运算、函数调用、变体构造、引用、where / match 等，只要最终求值结果落在上述 JSON 类型集合内（前面允许有指令、装饰器、注释、空白）。
