...
`#strict` 模式下，spread 源若不是 dict 字面量且没有 `<T>` 提示，
analyzer 会报 `MissingSpreadTypeHint`。
...
`#strict` 模式下，动态键缺少 `<T>` 时报 `MissingDynamicKeyTypeHint`。
...
- `#name ...` —— **指令**：声明 / 结构 / 元数据。完整集合是
  ...
  `#strict`（v1.3 起）。指令名是固定的、由宿主注册，不可由用户定义。
...
在文件根写一行 `#strict`，整个文件以及它通过 `#import` 触达的所有
模块都进入「严格静态推断」模式：
...
进入 strict 后，原本 analyzer 走 silent fallback 的位置（推不出来
时退化为 `Any` 让 runtime 兜底）会改报错：

- 没有类型提示的 spread（`{ ...e }`）→ `MissingSpreadTypeHint`
- 没有类型提示的动态键（`{ [k]: v }`）→ `MissingDynamicKeyTypeHint`
- typed spread / dynamic key 的 `<T>` 引用未声明的 schema →
  `UnresolvedSchema`
- 调用未在 `host_fn_signatures` 登记返回类型的 native fn →
  `StrictForbidsNativeReturn`

**传染性**：strict 是 entry 决定的；只在 entry 写一次，所有被它
import 的库（哪怕没写 `#strict`）也按 strict 校验。这样可以防止
silent fallback 从非 strict 库渗透进 strict entry。

完整的 strict 语义、新增诊断列表、`<T>` typehint 语法、`Dict<K, V>`
泛型，详见规范 §6.6。
