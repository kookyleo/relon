...
> 的函数；其中大部分（`sqrt`、`is_email`、`trim`、`split` 等）现已
> 编译四方，但仍有一批残留（`pow`、`to_json`、`unique`、`starts_with`
> 等）**只**存在于 tree-walker——没有 analyzer 签名，也没有任何编译
...
签名**，也**不存在于任何编译后端**（Cranelift / LLVM AOT、trace JIT）。
...
无 wasm 可移植 body（引擎 / seam 类 cap）。把它们当作参考求值器的
便利函数，而非可移植的语言 surface。
