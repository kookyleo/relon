...
| `std/string` | `split`、`join`、`replace`、`upper`、`lower`、`contains`、`glob_match` | 字符串操作 |
...
里有约 76 个 `register_pure_fn` 名字
（`crates/relon-evaluator/src/stdlib.rs`）；这一波 JSON-Schema 对齐
辅助函数的其余部分仍**只**注册在 tree-walker 里。它们**没有 analyzer
签名**，也**不存在于任何编译后端**（Cranelift / LLVM-native / wasm）。
在编译后端下调用、或指望它们被静态定型，行为都不会与 tree-walker 一致。
集合如下：

* 格式谓词：`is_ipv4`、`is_ipv6`
* 字符串：`starts_with`（自由函数形；方法形已有编译 registry 槽位）、
  `matches`（regex 引擎 cap）
* 字典：`select_keys`、`omit_keys`
* json / 日期：`to_json`、`parse_iso_date`

它们是 **tier-2 / 仅 tree-walker**，且这是**裁决过的设计边界**，
不是待办：`select_keys` / `omit_keys` / `parse_iso_date` 返回 Dict
（Dict 编译值已裁决不立项）；`to_json` 是复合类型 → String（同上
已裁决）；`is_ipv4` / `is_ipv6` 走 `core::net::Ipv*Addr::parse`、
`matches` 依赖 regex 引擎，均无 wasm 可移植 body（引擎 / seam 类
cap）。把它们当作参考求值器的便利函数，而非可移植的语言 surface。
