...
* 数值：`sqrt`、`round`、`floor`、`ceil`（标量 Float math，`sqrt`
  对 `Int` 参数自动 widen）；`multiple_of`（Int 形）、`in_range`
* 格式谓词：`is_uuid`、`is_email`、`is_uri`
* 字符串：`len`、`ends_with`、`replace`、`trim`、`trim_start`、
  `trim_end`、`split`
* 列表 / size：`size_in_range`（List 形，元素数取自 record header）

这一集合里有两个诚实 cap：`multiple_of` 的 Float 参数形（Cranelift/wasm
无原生 `%`/`Op::Mod(F64)`）与 `size_in_range` 的 String 参数形（数
Unicode 码点，需要 UTF-8 解码缝——可解，但当前未编 body）仍仅在
tree-walker 里，会响亮地回退到解释器，而不是悄悄编错值。
