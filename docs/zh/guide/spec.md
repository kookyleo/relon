...
完整指令集合（v1）：`#main(...)`、`#schema X Body`、`#enum X { ... }`、
`#import ... from "..."`、`#internal`、`#default`、`#expect`、`#msg`、
`#error`、`#brand X`、`#relaxed`（同义词 `#unstrict`）、`#derive`、
`#no_auto_derive`、`#native`、`#extend`。
...
面量；closure 体内的引用解析仍偏运行期。这些短板的补齐方向以工程
roadmap 体现。（capability 的静态可达性分析已落地：analyzer 的
`capability_check` 对调用 gated native fn 的位点发
`CapabilityRequired` 诊断。）
...
* 浮点运算遵守 IEEE-754；**禁止** fast-math、FMA 自动融合或编译期
  常量折叠产生不同舍入。**一个显式例外**：除数为零（含 Float
  `0.0` / `-0.0`）一律抛 `DivisionByZero`，**不**产生 IEEE-754 的
  `±inf` / `NaN`——这是规范的有意选择，让除零在所有后端都响亮失败。
...
* `string.split` 等基于「字符串」的操作以**字节**为单位（reference
  ...
  cluster）操作，必须由 host 通过 native fn 显式提供。**一个与
  Rust 不同的例外**：`split` 的分隔符不允许为空字符串——空分隔符
  报 `UnsupportedOperator`，而非像 Rust `str::split("")` 那样在每个
  字符边界切分。
...
每个 `#name ...` 指令必须满足下列六种 shape 之一。shape 由名字决
...
| Enum | `#enum Name { Variant, ... }` | `#enum Stat { Up, Down }` | Rust-like 枚举声明 |
...
或一次性授权全部（`Capabilities::all_granted()`）。这只允许作为宿主
显式、可审计的决定存在，不能成为 code review 看不见的隐式 trust。
公开的捷径 API 可以存在于 host-owned 脚本路径上，但名称和文档必须
把信任姿态写清楚（`--trust`、`*_trusted`、`TrustLevel::Trusted`）；
不可信脚本必须继续走沙箱入口，并只授予必要的最小能力。
...
| `MainReturnTypeMismatch` | 根表达式求值结果不匹配 `#main` 声明的 `-> ReturnType` |
| `UnsupportedOperator` | 无效操作或类型组合 |
| `ValidationError` | schema 校验失败（`ensure.*` / `#expect` 不满足）|
| `DivisionByZero` | 除数为零（含 Float `0.0` / `-0.0`，见 §2.3）|
| `CircularReference` | 引用求值形成环（如 `&sibling` 互相依赖）|
| `InvalidIdentifier` | 动态键等位置产生的名字不是合法标识符 |
| `IndexOutOfBounds` | 索引 / `substring` 等越界访问 |
| `EmptyList` | 需要非空列表的操作（如 `xs.max()`）收到空列表 |
| `Unsupported` | 所选执行后端不支持该构造（响亮报错，不静默回退）|
| `RemoteImportFailed` / `RemoteImportDenied` / `RemoteImportHashMismatch` | 远程 `#import` 拉取失败 / 未授权 / 完整性钉不匹配 |
| `ImportHashMismatch` / `ImportHashUnknownAlgorithm` / `ImportHashInvalidHex` | `#import` 完整性钉校验失败 / 算法名未知 / hex 非法 |
...
* `type(value)` — 返回值的类型名（`String`：`"Int"`、`"Float"`、
  ...
  此外内部值形态会返回 `"Schema"`、`"EnumSchema"`、`"Type"`、
  `"Wildcard"`——这四种值不可投影为 JSON，正常脚本数据流里见不到）。
...
| `std/value` | `default` | `None` 的 fallback |

稳定 stdlib surface 以
[稳定用户 API manifest](./stdlib.md#稳定用户-api-manifest) 为准。
`std/string.glob_match` 在首个公开版本中仍是 legacy/runtime-only，
没有进入稳定清单。
...
> 的函数；其中大部分（`sqrt`、`pow`、`unique`、`every` / `some`、
> `is_email`、`trim`、`split` 等）现已编译四方，但仍有一批残留
> （`to_json`、自由函数形的 `starts_with`、`select_keys` /
> `omit_keys`、`parse_iso_date`、`is_ipv4` / `is_ipv6`、`matches`）
> **只**存在于 tree-walker——没有 analyzer 签名，也没有任何编译
> 后端 IR 转换。依赖它们之前请先看 §6.7 的确切划分与 tier 说明。
...
一个 `.relon` 文件求值后得到**一个可投影为 JSON 的 Relon 值**——Object、Array、String、
Number、Bool，或投影为 JSON null 的 `None`。Root **可以是任意表达式**：dict / list / tuple literal、
...
`UnsupportedSchema`）。在静态侧，`#main(...) -> ReturnType` 写裸
`Closure` 会先被 `BareGenericContainer` 拦截（v1.7 裸泛型禁）；写
未声明的类型名报 `UnknownTypeName`；body 的静态类型与声明不符时
analyzer 的 `check_main_return` 报 `MainReturnTypeMismatch`。
...
5. 文件**没有 `#main(...)`** 时调用 `run_main` 报 `NoMainSignature`
   ——edge case 立即在边界截住。`eval_root` **不做**这项检查：对
   `#main` 文件调用 `eval_root` 会直接求值根表达式（形参没有绑定，
   引用它们会按未定义名报错）。
...
**按模块（文件级）**：strict 是**每个模块各管各的**——每个模块默认严格，只能用它**自己**头部的 `#relaxed` / `#unstrict` 退出，一个模块的指令只管它自己。入口的 `#relaxed` **不会**被印到它 `#import` 的库上：没写指令的库始终按严格校验，无论谁导入它；要放松必须在库文件自己头部写 `#relaxed`。整程序「不漏 `Any`」靠**边界拦截**保证——严格模块不会静态接受 `#relaxed` 依赖产出的 `Any`，会在**使用点**报错（被依赖的库既不被重新检查，也不被消费者放松）。这与 TypeScript / Rust / Sorbet / C# / Dart / JS 的按文件/按模块自选一致，Safe Haskell 则做成向内的 import 约束。
...
| `BareGenericContainer { type_name, context }` | v1.7：用户写了不带泛型参数的 `List` / `Dict` / `Closure` / `Fn` |
...
`Dict<String, V>`）、Rust-like `#enum` 声明、或自定义 `#schema`。「我接受任何
...
**严格模式下闭包参数的上下文推断豁免（R1）**：闭包参数无注解并非
一律报错——当**调用点签名**能钉住参数类型时（如 `xs.map((x) => ...)`
里 `x` 的类型由 `map` 的 `Closure<T, U>` 槽位 + 泛型统一推出），该
参数被视为「类型可静态导出」，不报 `ClosureParamTypeMissing`；只有
上下文也钉不住的参数才报错。
...
（`Enum` 不在此名单：它与 `Null` / `Unit` 一样属**保留类型名**，写
在类型位置报 `ReservedTypeName`，与裸泛型禁无关——和数据建模用
`#enum` 声明具体枚举。）

**唯一豁免（W7）**：`#main(...) -> Dict` 搭配 **dict 字面量 body**
时，裸 `Dict` 返回类型的 `BareGenericContainer` 诊断被豁免——此时
返回 schema 由 body 逐字段合成，形状是良定义的，不构成
`Dict<Any, Any>` 后门。参数位置与嵌套泛型槽位的禁令照常生效。
...
* **公开 enum 语法是 Rust-like `#enum`**：状态和 sum type 使用
  `#enum Stat { Up, Down }` 这类声明。
* **`Result<T, E>` / `Option<T>` 泛型替换**：以前
  `Result<Int, String> r: Ok("wrong")` 只有 runtime
  能抓住；v1.8 在分析阶段对变体字段做 `T -> Int, E -> String` 替换并
  递归校验，所有用户声明的带泛型 Rust-like `#enum` schema 同等享受
  这条流水线。`Result` / `Option` 的变体形状由 `seed_prelude_variants`
...
当前运行期动态键表达式不承诺继承同一 dict 的 sibling 作用域；用户文
档中的可运行示例应使用自足表达式，例如 `{ [<String> "user_" + "42"]:
"Alice" }`。
...
**已编译四方（tree-walk + Cranelift + LLVM-native + wasm，逐字节相等）。**
以下 stdlib 辅助函数现已 lower 进所有编译后端，并对 tree-walk oracle
逐字节验证一致（见 `SUPPORTED_SURFACE` 账本与
`no_fallback_over_supported_surface` 证明）：

* 数值：`sqrt`、`round`、`floor`、`ceil`（标量 Float math，`sqrt`
  对 `Int` 参数自动 widen）；`pow`（含 Int widen、负指数、上溢出到
  `inf`）；`multiple_of`（Int 形）、`in_range`
* 格式谓词：`is_uuid`、`is_email`、`is_uri`、`is_iso_date`
* 字符串：`len`、`ends_with`（自由函数形与 `s.ends_with(p)` 方法形
  共用同一 body，与 `starts_with` 方法形对称）、`replace`、`trim`、
  `trim_start`、`trim_end`、`split`
* 列表 / size：`size_in_range`（List 形，元素数取自 record header）；
  `count`（record-header 长度读取，任意元素类型）；`every` / `some`
  （短路谓词循环，`List<Int>` / `List<Float>`；空列表分别为
  vacuously true / false）；`unique`（O(N²) i<j 扫描，
  `List<Int>` / `List<Float>`；浮点相等取 OrderedFloat 语义——
  `NaN == NaN`、`-0.0 == 0.0` 均算重复）
* 列表方法形 reducer：`xs.sum()`（`List<Int>`，**checked**——首个
  溢出的部分和按元素序 trap `NumericOverflow`，与 `+` 运算符及
  `std/list` 的 reduce 版 `sum` 一致；历史上它曾是语言里唯一悄悄
  wrapping 的 Int 算术，现已修复）；`xs.max()` / `xs.min()`
  （`List<Int>`，空表 trap `EmptyList`；`min` 是 `max` 的精确镜像）

这一集合里的诚实 cap（都响亮回退或响亮拒编，绝不悄悄编错值）：
`xs.sum()` / `xs.max()` / `xs.min()` 的 `List<Float>` 元素形（留在
tree-walk fallback，编译后端响亮拒编）；`multiple_of` 的 Float 参数形（Cranelift/wasm 无原生
`%`/`Op::Mod(F64)`）；`size_in_range` 的 String 参数形（数 Unicode
码点，需要 UTF-8 解码缝——可解，但当前未编 body）；`every` / `some` /
`unique` 的 `List<String>` / `List<Bool>` 元素形；以及 `count` 的
count-empty FastInt 入口形（`count(range(0))`，无 buffer 操作数——
wasm 对象发射对它响亮拒绝，等价的 Buffer 入口源已四方验证）。

**遗留 runtime-only 注册不属于稳定 stdlib。** tree-walker 的内部
free-fn 注册表里仍保留一些历史 `register_pure_fn` 名字；它们没有
analyzer 签名，也没有编译后端槽位。即使绕过 analyzer 时参考求值器
能够调用它们，它们也**不是**可移植语言 surface。稳定的用户可见
stdlib 以 [标准库 manifest](./stdlib.md#稳定用户-api-manifest) 为准：
必须在该页文档化，并被 analyzer 的 stdlib-signature 表接受。

当前 runtime-only 名字由
`crates/relon-evaluator/src/stdlib_drift_tests.rs` 钉住：新增没有
analyzer 签名的 free fn 会让测试失败；某个遗留名获得正式签名时，也
必须从 allowlist 移出。这是防漂移机制，不是产品 tier。脚本不应依赖
这些名字获得可移植行为。
...
* 没有隐式 trust 让脚本绕开沙箱；宿主 opt-out 必须显式、可审计
