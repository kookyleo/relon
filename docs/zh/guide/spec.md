...
* **Tuple → List 折叠**：同质 tuple 仍可放进 `List<T>` 槽位（每个元素都
  ...
  `Tuple` **只**存在于 analyzer 的类型 lattice——runtime 没有
  `Value::Tuple` 变体；运行时 tuple 就是 `Value::List`（见 §6.7）。
...
* **Tuple 位置访问 `pair.0` / `pair.1`**：parser 一直就支持
  ...
  硬失败、`List<T>, Index(_)` → `T`。位置越界在严格模式下升级
...
* json / 日期：`to_json`、`from_json`、`parse_iso_date`、`size_in_range`
...
**`Tuple` 是仅静态类型；runtime 没有 `Value::Tuple` 变体。** tuple 只
活在 analyzer 的类型 lattice 里：list 字面量推断为
`Tuple<T1, T2, ...>`，`Tuple` 槽位在静态做 arity 与位置校验。运行时
tuple **就是** `Value::List`，没有专门的 tuple 值
（`crates/relon-evaluator/src/stdlib.rs`、
`crates/relon-evaluator/src/schema.rs`）。位置访问（`pair.0`）走普通
列表下标。
