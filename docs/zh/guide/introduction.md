...
* **标准库是语言的一部分**：`std/list`、`std/string`、`std/math`…
  ...
  注册——业务作者只依赖语言提供的稳定名字。
* **多档自动执行**：tree-walk / bytecode VM / Cranelift AOT /
  trace JIT 四档由 `Backend::Auto` 按热度自动切换——冷代码即时启动，
  热代码进入接近 LuaJIT trace tier 的快路；四道沙箱安检（bounds /
  trap / capability / resource）在所有档位完整保留。
...
Relon 的运行时是**分档自动切换**的：同一份 `.relon`，根据热度可以被
四种后端中的任意一种执行——tree-walk（解释执行）、bytecode VM、
Cranelift AOT、trace JIT。`Backend::Auto` 入口按 IR op 数量与调用次数
阈值自动选档，调用方不需要关心。

紧凑整数 hot loop 的 recorded trace 测得 **2.13 ns/iter**（trace JIT），
cached cold start **339 μs**（artefact 落盘后下次启动只走 dlopen，
不重新编译）。string / dict 等热路径目前仍在优化中，不代表所有 workload
都已贴近 LuaJIT。完整数据、对照基准与诚实的注解见
[性能与执行档位](./performance.md)。
...
- 标准库一览：[标准库](./stdlib.md)
- 性能与执行档位：[性能与执行档位](./performance.md)
