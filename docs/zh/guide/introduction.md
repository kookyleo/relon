...
- **沙箱默认安全**：`Capabilities` 控制文件读、求值预算、value 大小；原生函数按 `NativeFnGate` 声明的能力 bit 闸门放行，没有按名白名单。
