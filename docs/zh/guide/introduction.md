# 简介：什么是 Relon？

Relon 是一种**可执行的数据格式**：它的载荷就是业务逻辑本身。把校验
规则、计费公式、工作流、风控策略写成 Relon 文档，存进数据库或配置文
件，由嵌入式的 Rust 运行时**确定地**求值——同一份源码 + 同一份输入
永远得到字节级一致的结果。

> **一句话定位**：Logic as data — 把业务逻辑像 JSON 一样写一次、存
> 一次、传一次，在显式宿主授权下确定地求值。

## 这意味着什么（设计上的硬约束）

* **同源 + 同输入 → 同输出**：Dict 迭代序由 `BTreeMap` 保序、浮点
  走 IEEE-754 `f64`、不读环境变量、不依赖系统时间或 RNG。
* **无隐式 trust**：脚本零环境特权；FS、网络、原生函数
  都需要由 `Capabilities` 显式授权。宿主自有代码可以显式全开，但
  这份授权必须可审计；脚本不能自行绕过宿主允诺。安全边界见
  [威胁模型](./threat-model.md)。
* **标准库是语言的一部分**：`std/list`、`std/string`、`std/math`…
  随运行时一起发布，脚本通过 `#import` 即可使用，不需要宿主额外
  注册——业务作者只依赖语言提供的稳定名字。

<figure style="margin: 2rem auto; max-width: 720px; text-align: center;">
  <img src="/positioning.svg" alt="Relon 的双层作者模型示意图" style="width: 100%; height: auto;" />
  <figcaption style="margin-top: 0.75rem; font-size: 0.9rem; color: #64748b; font-style: italic;">双层作者模型：平台团队产出词表，业务团队拼装词表。</figcaption>
</figure>

## Relon 是什么

把 Relon 当成「为业务配置量身定做的小型 toolkit」最直观：

- **JSON-like 语法**：写起来像加了表达式、指令、装饰器、引用的 JSON。习惯 JSON 的人 5 分钟上手。
- **类型化 schema**：`#schema` 定义契约，支持 sum-type tagged enum、递归 schema、自定义校验消息、计算默认值。
- **宿主扩展**：Rust 端注册原生函数和装饰器；`.relon` 端写共享 schema/帮助函数；两侧通过 `#import` 拼起来。
- **默认能力姿态**：`Capabilities` 命名并授予文件、网络、时钟、环境变量、随机数这类宿主权限；`ResourceBudget` 描述求值预算。原生函数按 `NativeFnGate` 声明的 capability bit 闸门放行，没有按名白名单。

## 谁在写什么——双层作者模型

Relon 的设计假设有两类作者：

| 角色 | 工作产出 | 关心的事 |
| --- | --- | --- |
| **平台 / 框架团队** | Rust 扩展（原生函数、装饰器插件） + `.relon` 库（不带 `#main`） | 暴露稳定的「业务词表」，把领域规则编码进 schema 与装饰器 |
| **业务 / 产品团队** | `.relon` 入口文件（需要宿主输入时带 `#main(...)`） | `#import` 平台库，写贴近 JSON 的配置组合，错误尽早被类型/校验拦截 |

文件是否带 `#main(...)` 决定它的用法：带 `#main` 的是「入口程序」
（必须由宿主推参数才能跑）；不带 `#main` 的是纯配置 / 库文件，可以
被宿主直接求值，也可以被其它文件 `#import`。平台团队通常写库文件；
业务团队如果不需要宿主输入，也可以写一个无 `#main` 的纯配置入口。

## 30 行带你走一遍

以下示例覆盖了 `#schema`、sum-type tagged enum、计算默认值、宿主接入。

**`platform/notify.relon`**（平台团队的库，无 `#main`）：

```relon
#enum Channel {
    Email { address: String, subject: String },
    SMS { phone: String },
    Push
}

{
    // 业务通用的「带主体的通知」契约 + 计算默认值
    #schema Notification {
        Channel via: *,
        String title: *,
        #default (self) => "[" + self.title + "]"
        String summary: *
    }
}
```

**`app/main.relon`**（业务团队的纯配置入口；需要宿主输入时再加
`#main(...)`）：

```relon
#import * from "../platform/notify.relon"
{
    Notification welcome: {
        via: Channel.Email { address: "user@x.com", subject: "Hi" },
        title: "Welcome"
    }
}
```

宿主端三行 Rust 拿到 JSON：

```rust
// 入口带 #import，要读本地文件——这是宿主自己的脚本，用 *_trusted 入口。
let json = relon::json_from_file_trusted("app/main.relon")?;
println!("{}", serde_json::to_string_pretty(&json)?);
```

> 默认入口 `json_from_file` 使用沙箱姿态：脚本零环境特权，`#import`
> 读文件系统会被拒绝。本例的 `#import` 需要文件读权限，所以用
> `json_from_file_trusted`（仅用于宿主自己拥有的脚本）；跑不可信脚本
> 时请收窄显式 capability，并使用合适的 VM / 进程边界，详见
> [威胁模型](./threat-model.md) 和 [沙箱与权限](./sandbox.md)。

输出（注意 `Email` 这一层是 sum-type 的**外部标签**形式）：

```json
{
  "welcome": {
    "summary": "[Welcome]",
    "title": "Welcome",
    "via": { "Email": { "address": "user@x.com", "subject": "Hi" } }
  }
}
```

## Relon 不是什么

为了避免误解，下面这些**不在路线图上**：

- ❌ **多格式输出**：不会生成 YAML/TOML/XML——[Pkl](https://pkl-lang.org/) 是可以的。
- ❌ **通用脚本语言**：没有 IO、没有循环语句、没有副作用——不要拿来替代 Lua/Starlark。
- ❌ **纯约束验证器**：Relon 既描述也求值；只想做约束，[CUE](https://cuelang.org/) 更合适。
- ❌ **总函数 / 纯函数主义**：求值会失败、closure 不强求 totality——这不是 [Dhall](https://dhall-lang.org/)。
- ❌ **跨语言原生类型/装饰器注册**：首个公开版本只把 Rust 嵌入作为稳定面。其它语言边界即使后续提供，也应是「JSON 进 JSON 出」或 host-fn 回调这类窄接口；不会承诺 Python/Node 端的 schema 注册。
- ❌ **多环境分支原语**：没有 `dev/staging/prod` 切换关键字——用普通的 `match` / `if` 表达式即可。

## 下一步去哪儿

- 语法基础：[基础语法](./syntax.md)
- 写契约：[类型与契约（Schema）](./types.md)
- 模块与入口程序：[模块与作用域](./modules.md)
- 嵌入到 Rust 宿主：[嵌入宿主](./host-integration.md)
- 跑不可信脚本：[沙箱与权限](./sandbox.md)
- 标准库一览：[标准库](./stdlib.md)
