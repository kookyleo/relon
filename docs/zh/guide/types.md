...
Relon 内部把 enum value 表示为带标签的 `Value::Dict`：`brand` 是变体名，`variant_of` 是 enum 名。进入具体 variant 分支后，payload 字段访问是扁平的：
...
msg match {
    Email: msg.address, // 没有 .Email 这一层
    SMS: msg.phone,
    Push: ""
}
```

注意：上面成立的前提是 `msg` 的静态类型已知（例如来自 `#main`
签名、schema 字段或 typed binding），并且代码已经通过 `match` 进入
某个具体 variant。当前 analyzer 不会从同一 dict 里的 sibling 字面量
反推出 enum payload 类型；需要这种写法时，请给值一个明确入口类型。
...
xs.map((Int x) -> Stat => x > 0 ? Stat.Up : Stat.Down)
```
...
| 诊断 | 触发条件 |
| --- | --- |
| `NonExhaustiveMatch` | 缺少变体且没有 `_` 通配符 |
| `UnknownVariant` | 写了不存在的变体名 |
| `DuplicateMatchArm` | 同一个变体名出现了两次 |

无法静态推断时，相关检查交给运行期。想要不写穷尽分支，可以加 `_: ...` 通配符。
...
```relon
{
    ...
    #schema ButtonConfig {
        ...
        Int width: (Int w) -> Bool => w >= 10 && w <= 100,
...
解析器也保留了历史上的字段位置形态 `#schema Name: { ... }`，但当前
analyzer 对这种形态里的谓词闭包仍会走普通字段路径，容易得到不符合
直觉的类型错误。首个公开版本的文档、示例和 formatter 都只推荐上面
这种独立声明；如果你在旧代码里见到字段位置形态，迁移为独立声明即可。
...
- **可选形态**：`#brand Option<Weather>`——和字段级 `Option<Weather> w: ...` 行为一致，`None` 放行，其它走原类型校验。
...
这个例子展示的是「类型可以递归引用自身」。当前 strict analyzer 对源码中
直接写出的深层 `List<Menu>` 树字面量仍比较保守；复杂树通常由 host
通过 `#main(...)` 推入 JSON，再按 `Menu` schema 校验。实现层面递归
校验深度上限为 20 层，远超绝大多数业务嵌套——如果你撞上了，多半是
数据形状出问题，先反思 schema。
...
```relon
{
    #schema Server {
        ...
        Int port: (Int p) -> Bool => p > 0 && p < 65536
    },
...
```relon
{
    #schema User {
        ...
        // 2. 可选（Option<T>）：字段可以缺失；没有值请显式用 None
        Option<String> bio: *,
