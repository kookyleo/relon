...
Relon 标准库是**规范的一部分**——任何 conformant 运行时都必须按相同
语义实现下列模块，脚本通过 `#import <bindspec> from "std/<name>"` 引入。
...
`ensure.one_of` 等。这些是 schema 系统的实现细节，spec §6.3 强制
所有 conformant 运行时按相同语义提供——但**不属于用户脚本应直接
调用的 API**。在你自己的 schema 里通过 `#expect` 指令使用即可。
...
> 在 spec 冻结这些模块之前，宿主端可以用 `register_fn_with_caps`
> 自己注册——但这样的脚本依赖宿主、不再 portable，会失去
> Logic-as-Portable-Data 的承诺。详见
