...
{
    List<Int> doubled:
        list.map([1, 2, 3], (Int x) -> Int => x * 2), // [2, 4, 6]
    List<Int> evens:
        list.filter(range(10), (Int x) -> Bool => x % 2 == 0),
    Int sum:
        list.reduce([1, 2, 3], 0, (Int acc, Int x) -> Int => acc + x),
    Bool has_two: list.contains([1, 2, 3], 2),        // true
    Int total: list.sum([1, 2, 3]),                   // 6
    Int mean: list.avg([1, 2, 3]),                    // 2
    Int n: list.len([1, 2, 3]),                       // 3
    Int head: list.first([10, 20, 30]),               // 10
    Int tail: list.last([10, 20, 30]),                // 30
    List<Int> compact_empty: list.compact([None]),    // []
    List<Int> flat: list.flatten([[1, 2], [3]])       // [1, 2, 3]
}
```

`list.compact` 的主要输入通常来自宿主推入的 `List<Option<T>>` 或 schema
字段；当前 strict analyzer 会拒绝源码里的混合 list 字面量
`[1, None, 2]`，因为 list 字面量本身必须同质。
...
{
    Dict<String, Int> base: { a: 1 },
    Dict<String, Int> over: { b: 2 },
    Dict<String, Int> merged:
        dict.merge(&sibling.base, &sibling.over),      // { a: 1, b: 2 }
    List<String> keys: dict.keys(&sibling.merged),     // ["a", "b"] (BTreeMap 序)
    List<Int> values: dict.values(&sibling.merged),    // [1, 2]
    Bool has_a: dict.has_key(&sibling.merged, "a")     // true
}
```
...
#import string from "std/string"
{
    ...
    String fixed: string.replace("hi world", "world", "relon"),
    String upper: string.upper("relon"),            // "RELON"
    String lower: string.lower("RELON"),            // "relon"
    Bool has_x: string.contains("hello", "ell")     // true
}
```
...
{
    Float a: math.abs(-3.0),     // 3.0
    Int hi: math.max(2, 5),      // 5
    Int lo: math.min(2, 5),      // 2
    Int bound: math.clamp(15, 0, 10) // 10
}
```
...
{
    String a: value.default(None, "fallback"), // "fallback"
    Bool b: value.default(false, true)         // false（仅 None 触发 fallback）
}
```
