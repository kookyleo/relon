...
| `stat(path)` | `Dict{size: Int, is_dir: Bool}` —— 文件元信息 | `reads_fs` | 标准 WASI preview1 `path_filestat_get` |
...
`stat(path)` 按同一沙箱根读取路径的文件元信息为 `{size: Int, is_dir: Bool}`
dict(同样拒绝逃逸)。`size` 是文件字节长度,`is_dir` 是该路径是否为目录。
`stat` 在四后端——tree-walk、cranelift-native、llvm-native、wasm32——逐字节
相等:wasm 臂降成标准 preview1 `path_filestat_get` import(固定 64 字节
`filestat` 结构——无 dirent 流),从中读取 `filetype` 与 `size`,故任意现成
WASI host 均可运行。
