...
如果觉得 schema 与 `@input(...)` 隔了一层 dict 体不太顺眼，可以用
**根级 `@schema(Name={...})` 装饰器糖**（spec §6.4.1）把 dict-body
形态的 schema 搬上来：
...
@input(user=User)
{
    @schema Post: { String title: * },
    @schema PostList: List<Post>,
    summary: f"${input.user.name}",
    eligible: input.user.tier == "gold"
}
```

根级糖里的 `Name=Body` 只接受 dict 字面量或 `Enum<...>`；非这两种
形态的 schema（如 `List<Post>` 这种类型别名）保留在 dict 体内、用
字段形 `@schema X: ...` 声明即可——两种形态可以并存。

两种写法语义完全一致——根级糖等价于把 `Name` 声明为根 dict 的
`@private @schema` 字段。选哪种是团队风格问题。
