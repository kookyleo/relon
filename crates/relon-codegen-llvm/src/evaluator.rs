...
/// `Op::ReadDir`). These are guarded by an `Op::CheckCap` reading the
/// buffer entry's trailing `caps` slot, so a module that uses them must
/// take the buffer-protocol entry even when its `#main` schema is
/// otherwise fast-path eligible.
fn ir_uses_capability_primitive(ir: &relon_ir::ir::Module) -> bool {
    ...
    fn body_uses(body: &[TaggedOp]) -> bool {
        body.iter().any(|t| match &t.op {
            Op::ReadClock | Op::ReadRandom | Op::ReadFile | Op::ReadDir => true,
            Op::If {
                then_body,
                else_body,
                ..
