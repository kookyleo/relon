...
use crate::directive_names::{ENUM, SCHEMA};
use crate::schema::{is_reserved_type_name, lower_schema_pure_with, record_schema_methods};
use crate::tree::AnalyzedTree;
use relon_parser::{DirectiveBody, Expr, Node, Operator, TokenRange, INTERNAL_ENUM_TYPE_NAME};
...
pub struct RootSchemaDecl {
    ...
    /// (`(...)`), or the internal body generated by `#enum`. The evaluator builds
...
pub fn collect_root_schemas(root: &Node, tree: &mut AnalyzedTree) {
    ...
    for dir in &root.directives {
        if dir.name != SCHEMA && dir.name != ENUM {
            continue;
        }
        ...
        // Validate the value's shape. Anything else (literals, numbers,
        // references) would not produce a schema at runtime, so reject it
        // eagerly. Accepted body shapes:
        //   * `Dict { ... }` literal — a struct schema body.
        //   * `Tuple (...)` literal — a positional tuple schema body.
        //   * internal `#enum` body generated by the parser.
