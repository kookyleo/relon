...
use relon_parser::{
    ...
    TokenRange, TypeNode, INTERNAL_ENUM_TYPE_NAME,
};
use std::sync::Arc;
...
pub struct SchemaDef {
    ...
    /// Tagged-enum variants, populated for `#enum X { A { ... }, B }`. When
    /// non-empty, `fields` and `bases` are unused — the schema is
    /// consumed via variant construction and pattern matching instead
    /// of dict validation.
...
fn check_schema_type_name(
    ...
    if t.path.len() == 1 {
        let head = &t.path[0];
        if is_reserved_type_name(head) {
            return;
        }
        if relon_parser::is_builtin_type_name(head) || matches!(head.as_str(), "Option" | "Result")
        {
            return;
        }
...
fn visit_field(key: &TokenKey, value: &Node, tree: &mut AnalyzedTree) {
    ...
    let mut name = None;
    let mut name_range = value.range;
    let mut generics = Vec::new();
    match key {
        TokenKey::String(s, range, _) => {
            name = Some(s.clone());
            name_range = *range;
        }
    ...
    if let Some(schema_name) = name.as_ref() {
        if is_reserved_type_name(schema_name) {
            tree.diagnostics.push(Diagnostic::ReservedTypeName {
                type_name: schema_name.clone(),
                context: "#schema name".to_string(),
...
pub fn lower_schema_pure_with(
    ...
    if let Some(schema_name) = name.as_ref() {
        if is_reserved_type_name(schema_name) {
            return (
                None,
                vec![Diagnostic::ReservedTypeName {
                    type_name: schema_name.clone(),
                    context: "#schema name".to_string(),
...
fn walk_schema_body(node: &Node, def: &mut SchemaDef, tree: &mut AnalyzedTree) -> bool {
    match &*node.expr {
        // Internal body generated by `#enum X { ... }`.
        Expr::Type(t) if t.path.len() == 1 && t.path[0] == INTERNAL_ENUM_TYPE_NAME => {
...
/// Lower the internal `#enum` schema body. The parser stores each variant
/// alternative as a `TypeNode` with `variant_fields = Some(...)`; public
/// source cannot write this body directly.
fn lower_enum_body(t: &TypeNode, def: &mut SchemaDef, tree: &mut AnalyzedTree) -> bool {
    if !t.generics.iter().all(|g| g.variant_fields.is_some()) {
        tree.diagnostics.push(Diagnostic::SchemaBodyNotDict {
            found: "invalid enum body".to_string(),
            range: span_of(t.range),
        });
        return false;
    }
...
mod tests {
    ...
    #[test]
    fn reserved_schema_names_are_rejected() {
        let tree = analyze_str(
            r#"
                #schema Null { Int x: * }
                { #schema Unit { Int y: * } }
        ...
        assert!(tree.has_errors(), "{:?}", tree.diagnostics);
        assert!(tree.diagnostics.iter().any(|d| {
            matches!(d, Diagnostic::ReservedTypeName { type_name, context, .. }
                if type_name == "Null" && context == "#schema name")
        }));
        assert!(tree.diagnostics.iter().any(|d| {
            matches!(d, Diagnostic::ReservedTypeName { type_name, context, .. }
                if type_name == "Unit" && context == "#schema name")
        }));
    }
    ...
    #[test]
    fn non_dict_schema_body_is_rejected() {
        let tree = analyze_str(
            r#"{
                #schema Mixed 1
            }"#,
        ...
        assert!(tree.has_errors(), "{:?}", tree.diagnostics);
        assert!(tree.diagnostics.iter().any(
            |d| matches!(d, Diagnostic::RootSchemaInvalidValue { name, .. } if name == "Mixed")
        ));
    }
}
