...
///
/// F5 lifted `List<List<String>>`; F6 lifted the deep nested-schema field
/// chain to a pointer-array leaf (`o.inner.tags`); F7 lifted the element
/// sub-record carrying `List<Schema>` / `List<List>` fields (recursive to
/// any depth). The only shape left is the `Dict<_,_>` param analyzer
/// dead-end (no input decode path).
#[test]
