...
use relon_codegen_cranelift::object_cache_integration as cache_int;
use relon_codegen_cranelift::vtable::{populate_vtable, VTABLE_SYMBOL};
...
fn cached_cold_start_phase_breakdown_prints() {
    ...
    let sandbox = relon_codegen_cranelift::SandboxConfig::default();
    ...
    let schema_path =
        relon_codegen_cranelift::schema_cache::schema_cache_path_for(cache.path(), source_hash);
    let t1 = Instant::now();
    let schema_bytes = std::fs::read(&schema_path).expect("schema read");
    let schema_entry = relon_codegen_cranelift::schema_cache::deserialize(
        &schema_bytes,
        &source_hash,
...
fn phony_metadata(
    source: &str,
    sandbox: &relon_codegen_cranelift::SandboxConfig,
